   ...
   const cid=location.state.session;
    ...
    useEffect(() => {
        const fetchUnits = async () => {
          try {
            const response = await fetch('http://localhost:3000/unit-getAllUnits');
            const data = await response.json();
            setUnits(data.units);
    ...
    const handleUnitSubmit = async(event) => {
        ...
        if (name.trim() !== '') {
            ...
            if (isDuplicate) {
              alert('Duplicate unit name. Please enter a unique name.');
            } 
         else if (editIndex === -1) {
         // try{
          const response=fetch('http://localhost:3000/unit-create', {
            method: 'POST',
            headers: {
      ...
      //  }
        ...
        */
            ...
            const response=fetch('http://localhost:3000/unit-update', {
            method: 'POST',
            headers: {
           'Content-Type': 'application/json',
            },
      ...
      const handleDeleteUnit = async(index) => {
        const response = await fetch(`http://localhost:3000/unit-getOneUnitsByName/${units[index]}`);
        const result = await response.json();
        const response1=fetch('http://localhost:3000/unit-delete', {
            method: 'POST',
            headers: {
           'Content-Type': 'application/json',
            },
  ...
  */
    ...
    return(
      ...
      <div className="bg-lime-100 px-4 py-2 mb-4 rounded-full text-center">
        <h1 className="text-lg">ADD UNIT</h1>
      </div>
      <div >
      <form onSubmit={handleUnitSubmit} className="flex mb-4">
        ...
        >
          {editIndex === -1 ? 'Add Unit' : 'Update Unit'}
        </button>
      ...
      <div  style={{backgroundColor: "#4d4dff", color: "white", padding: "4px", marginBottom: "4px", textAlign: "center", borderRadius: "35px", textAlign: "center",marginLeft:'10vw',marginRight:'10vw' }}>
        <h1 className="text-lg">LIST OF UNIT</h1>
      </div>
  ...
  <table style={{ width: '70%', margin: '0 auto', borderCollapse: 'collapse' }}>
    <thead>
      <tr style={{ borderBottom: '1px solid gray' }}>
        ...
        <th style={{ padding: '8px', textAlign: 'left', backgroundColor: '#a6a6a6', color: 'black' }}>Unit</th>
